...
repertoire = 'OutputPythonScript/ModalPINN_VS_ExpMes_desyncPitot_'+ d.strftime("%Y_%m_%d-%H_%M_%S") + '__' +str(r)
...
layers = [2,20*Nmodes,20*Nmodes,Nmodes]
...
Delta_t_np_pitot = 0.*np.random.uniform(low=0.0,high=2*np.pi/omega_0, size=Nxpitot)
...
t_tf_mes_pitot_resync_unflatten = tf.convert_to_tensor([[ t_tf_mes_pitot_unflatten[t,k] - Delta_t_tf_pitot[k] for k in range(Nxpitot)] for t in range(Ntimes)])
...
x_int,y_int,t_int,s_train,xmes_pitot,ymes_pitot,tmes_pitot,umes_pitot,vmes_pitot,pmes_pitot,xmes_cyl,ymes_cyl,tmes_cyl,umes_cyl,vmes_cyl,pmes_cyl,Delta_t_np_pitot_applied = ltd.training_dict(Nmes,Nint,Nbc,filename_data,geom,Tintmax=1e2,data_selection = 'cylinder_pitot',desync=True, multigrid=multigrid,Ngrid=Ngrid)
Ncyl = len(xmes_cyl)
...
Delta_t_tf_pitot_found_o = sess.run(Delta_t_tf_pitot)
err_rms_resync = np.sqrt(np.mean(np.square((r_div_eucli(Delta_t_tf_pitot_found_o-Delta_t_np_pitot_applied,2*np.pi/omega_0)))))
err_rms_resync_normalized = err_rms_resync/np.sqrt(np.mean(np.square(Delta_t_np_pitot_applied)))
print('Err RMS Resynchro : %.3e'%(err_rms_resync))
print('Err RMS Resynchro normalized : %.3e'%(err_rms_resync_normalized))
print('RMS delta_t applied %.3e'%(np.sqrt(np.mean(np.square(Delta_t_np_pitot_applied)))))

print('Delta_t_np_pitot : \n '+str(Delta_t_np_pitot_applied))
...
err_resync_pitot = r_div_eucli(Delta_t_tf_pitot_found_o-Delta_t_np_pitot_applied,2*np.pi/omega_0)

size_resync = np.log10(1.+10.*np.abs(err_resync_pitot))

plt.figure()
plt.scatter(xpitot,ypitot,c=np.log10(np.abs(err_resync_pitot)),marker='o',s=1.+size_resync)
plt.colorbar()
...
# =============================================================================
# Export Dlta_t applied and found
# =============================================================================


print('Saving Delta t...')

filename_Delta_t = repertoire + '/Delta_t_applied_and_found.pickle'

pcklfile_Delta_t = open(filename_Delta_t,'ab+')
pickle.dump([xpitot,ypitot,Delta_t_tf_pitot_found_o,Delta_t_np_pitot_applied],pcklfile_Delta_t)
pcklfile_Delta_t.close()
print('Delta t exported in '+repertoire)
