...
repertoire = 'OutputPythonScript/ModalPINN_VS_DenseMes_'+ d.strftime("%Y_%m_%d-%H_%M_%S") + '__' +str(r)
...
print('Neurons per layer : %d' % (args.WidthLayer))
...
opt_LBFGS = nnf.declare_LBFGS(Loss,maxit=10000,ftol=1e-8)
...
x_int,y_int,t_int,s_train,xmes,ymes,tmes,umes,vmes,pmes = ltd.training_dict(Nmes,Nint,Nbc,filename_data,geom,Tintmax=1e2,data_selection = 'all',desync=False, multigrid=multigrid,Ngrid=Ngrid,stdNoise=stdNoise,cut=True,method_int='uniform')
Nmes = len(xmes)
Tmin = 400.
...
data_export = [Nmodes,args.WidthLayer, Nelts(layers),stdNoise,loss_validation_o,loss_eqs_int_o]
