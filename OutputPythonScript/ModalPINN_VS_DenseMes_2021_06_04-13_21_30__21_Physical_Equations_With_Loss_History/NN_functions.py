...
def NN_time_uv(x,y,t,weights,biases,geom,omega_0,trunc_mode=None):
    ...
    trunc_mode : int (or None) : if int select only the trunc_mode first mode given. Else if None, use all modes
    '''
...
def customgrad(fgrad,xgrad):
    '''
    Prend en entrée un tenseur de dimension [1,Nint,Nmode] de type float32
    Retourne un tenseur de même dimension et type
    '''
    Nmode = int(fgrad[0,0,:].shape[0])
    fgrad_xgrad =  [tf.gradients(fgrad[:,:,k], xgrad)[0] for k in range(Nmode)]
